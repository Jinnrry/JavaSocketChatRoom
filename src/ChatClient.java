...
public class ChatClient extends JFrame implements ActionListener {
	JFrame clientFrame = new JFrame("Socket网络聊天室");
		...
		try {
			if (obj.equals(btnLogin)) {
				if ((txtServerIP.getText().length() > 0)
					...
					connectServer();
					strSend = "login|" + txtName.getText() + "|"     //发送登录信息
				...
				if ((txtName.getText().length() > 0)
						&& (txtPassword.getText().length() > 0)) {
					connectServer();
					strSend = "reg|" + txtName.getText() + "|"     //发送注册信息
							+ String.valueOf(txtPassword.getPassword());
					out.println(strSend);
					initLogin();
				}
					...
					try {
						...
						if (strKey.equals("talk")) {
							...
							while (st.hasMoreTokens()) {
								...
								boolean add=true;   //标志是否添加
								int len=listOnline.getItemCount();
								for(int i=0;i<len;i++){
									if(strOnline.equals(listOnline.getItemAt(i))){
										add=false;                      //如果列表中已经存在这个用户了就不添加
									}
								}
								if (add)
